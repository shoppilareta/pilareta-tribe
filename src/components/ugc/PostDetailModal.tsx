...
export function PostDetailModal({
  ...
  const { post, loading, fetchPost, likePost, unlikePost, savePost, unsavePost, addComment, deletePost, editPost } = usePost();
  ...
  const handleSaveEdit = useCallback(async () => {
    if (!post) return;
    setSaving(true);
    const result = await editPost(post.id, editCaption);
    if (result.success) {
      setIsEditing(false);
      if (onInteraction) {
        onInteraction(post.id, { caption: editCaption.trim() || null });
      }
    }
    setSaving(false);
  }, [post, editCaption, editPost, onInteraction]);
